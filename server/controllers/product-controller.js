...
exports.getProducts = async (req, res, next) => {
    try {
        ...
        const query = `SELECT * FROM products WHERE categoryId = ? AND (name LIKE '%${name}%');`;
        const result = await database.execute(query, [req.query.categoryId]);
        const response = {
...
exports.postProduct = async (req, res, next) => {
    try {
        ...
        const result = await database.execute(query, [req.body.name, req.body.price,
             req.file.path, req.body.categoryId]);
